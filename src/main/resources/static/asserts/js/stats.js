...
stats_obj={
			...
			$.ajax({
				...
				success:function(data){
					if(data.type=='success'){
						setOption('日销售退货统计',stats_obj.getKeys(data.sellData),stats_obj.getValues(data.sellData),stats_obj.getRebackValues(stats_obj.getKeys(data.sellData),data.sellRebackData));
					}
					else{
		...
		statsMonth:function(){
			$.ajax({
				...
				success:function(data){
					if(data.type=='success'){
						setOption('月销售退货统计',stats_obj.getKeys(data.sellData),stats_obj.getValues(data.sellData),stats_obj.getRebackValues(stats_obj.getKeys(data.sellData),data.sellRebackData));
					}
					else{
						$.messager.alert('信息提示',data.msg,'warning');
					}
...
//setOption('日销售退货统计',['A','B','C','D','E'],[10,22,30,40,50,52],[5,8,12,23,25,30]);
stats_obj.statsDay();
